...
function openBottomSheet(spot) {
  ...
  document.getElementById("poolType").textContent = unknownHeight
    ? `${spot.type} • Sprunghöhe noch unbekannt`
    : `${spot.type} • Max. ${spot.height}m Turm`;

  // Report-Button: bei fehlender Höhenangabe als einladende Mitmach-Aktion
  // hervorheben statt als neutrale "Änderung melden"-Option
  const reportBtn = document.getElementById("openReportBtn");
  if (reportBtn) {
    if (unknownHeight) {
      reportBtn.textContent = "🙋 Sprunghöhe bekannt? Jetzt ergänzen";
      reportBtn.style.background = "rgba(16, 185, 129, 0.15)";
      reportBtn.style.borderColor = "#10b981";
